    ...
    <div className="relative flex flex-col items-center text-center p-4 sm:p-8 w-full min-h-[450px] justify-between overflow-hidden rounded-4xl">
      ...
      <div className="space-y-15 mb-6 z-10 w-full px-2">
        ...
        <p className="text-slate-500 text-sm sm:text-base font-medium leading-relaxed mb-5">
          You just crushed <span className="text-blue-700 font-bold bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100">{exerciseName}</span>.
          <br className="hidden sm:block mt-1" /> Take a breath, and let's keep this momentum going.
        </p>
      </div>
      ...
      <div className="w-full mt-auto z-10 flex flex-col items-center gap-5">
        ...
        >
          Continue Journey
          <ArrowRight size={25} className="opacity-70 group-hover:translate-x-2 transition-transform" />
        </button>
      </div>
    </div>
  );
      ...
      <div className="relative z-10 space-y-8">
        ...
        <div className="relative inline-block">
          ...
          <div className="relative bg-gradient-to-b from-yellow-100 to-orange-100 p-8 rounded-full inline-flex items-center justify-center ring-8 ring-yellow-50 shadow-2xl">
            ...
            <Sparkles className="absolute -top-2 -right-2 text-yellow-500 animate-pulse" size={32} />
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            <p className="text-blue-600 font-bold text-lg uppercase tracking-widest">{sessionTitle}</p>
            <div className="h-[1px] w-8 bg-slate-200" />
          </div>
        ...
        <div className="grid grid-cols-3 gap-3 relative">
          <div className="bg-white p-4 rounded-2xl border-2 border-green-100 shadow-sm transition-transform hover:scale-105">
            <p className="text-[10px] font-black text-green-500 uppercase mb-1">Status</p>
          ...
          <div className="bg-gradient-to-br from-blue-600 to-blue-700 p-4 rounded-2xl shadow-lg shadow-blue-200 transition-transform hover:scale-105">
            <p className="text-[10px] font-black text-blue-100 uppercase mb-1">Effort</p>
          ...
          <div className="bg-white p-4 rounded-2xl border-2 border-purple-100 shadow-sm transition-transform hover:scale-105">
            <p className="text-[10px] font-black text-purple-500 uppercase mb-1">Streak</p>
...
export default function OverviewDashboard() {
  ...
  const handleCloseExSuccess = () => {
    ...
    if (data) {
      const currentWeekSessions = data.exercise_plan.weekly_schedule[0]?.sessions || [];
  ...
  const displayedSession = currentWeekSessions.find((s) => s.day === selectedDay) || currentWeekSessions[0];
  ...
  return (
    <div className="min-h-screen bg-white p-4 md:p-12 font-sans text-slate-900">
      <main className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-12">
        <div className="lg:col-span-2 space-y-6 md:space-y-8">
          <div className="flex justify-between items-end px-2">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-blue-800 tracking-tight">Overview</h1>
              <p className="text-slate-400 font-medium text-sm">Week {progress.current_week} - Day {selectedDay}</p>
            </div>
            <button onClick={() => fetchPlanData()} className="p-2 text-slate-300 hover:text-blue-600 transition-colors">
              <RefreshCw size={20} />
            </button>
          </div>
          ...
          <div className="bg-blue-100/60 rounded-[1.5rem] md:rounded-[2rem] p-6 md:p-8 flex justify-between items-center border border-blue-100 relative overflow-hidden">
            <div className="z-10 relative">
              <h2 className="text-2xl md:text-3xl font-bold text-slate-800">{plan_info.user_name}</h2>
              <p className="text-blue-600 font-semibold text-sm md:text-base mt-1">Goal: {plan_info.primary_goal}</p>
            </div>
            <div className="text-right z-10">
              <p className="text-4xl md:text-5xl font-bold text-blue-400">{Math.round(progress.completion_percent)}%</p>
          ...
          <div className="space-y-3">
            <h3 className="text-lg font-bold text-slate-800 ml-2 flex items-center gap-2">
              <span className="bg-blue-100 text-blue-600 text-xs px-2 py-1 rounded-md uppercase">
                {displayedSession?.duration_min ? `${displayedSession.duration_min} min` : "Plan"}
              </span>
            ...
            {displayedSession?.exercises.map((ex) => (
              <div
                key={ex.id}
                className={`bg-white p-4 md:p-6 rounded-2xl md:rounded-3xl border shadow-sm flex items-center justify-between transition-all ${ex.status.trim() === "completed" ? "border-green-100 bg-green-50/30" : "border-slate-100 hover:border-slate-200"}`}
              >
                <div className="flex items-center gap-4 md:gap-6">
                  <div className={`p-2 md:p-3 rounded-full ${ex.status.trim() === "completed" ? "bg-green-100 text-green-600" : "bg-slate-50 text-slate-300"}`}>
                    <PlayCircle size={20} className="md:w-6 md:h-6" />
                  </div>
                  <div>
                    <h4 className={`font-bold text-sm md:text-base ${ex.status === "completed" ? "text-green-900" : "text-slate-800"}`}>{ex.name}</h4>
                    <p className="text-[10px] md:text-[11px] text-slate-400 font-bold uppercase">{ex.sets} sets • {ex.reps} reps</p>
                  </div>
                </div>
                <button
                  onClick={() => { setActiveEx(ex); setIsModalOpen(true); }}
        ...
        <aside className="bg-white rounded-[2rem] p-6 md:p-10 border border-slate-100 shadow-xl h-fit sticky top-6 hidden lg:block">
          <h3 className="font-bold text-slate-800 text-xl mb-8">Roadmap</h3>
          <div className="relative space-y-10">
            ...
            {currentWeekSessions.map((session, idx) => {
              ...
              return (
                <button key={idx} onClick={() => setSelectedDay(session.day)} className="relative pl-10 w-full text-left transition-all group">
                  <div className={`absolute left-0 w-6 h-6 rounded-full border-4 border-white z-1 shadow-sm ${isActive ? "bg-blue-600" : "bg-white border-blue-200"}`} />
                  <div className={isActive ? "opacity-100" : "opacity-60"}>
                    <p className="text-[10px] font-bold text-slate-400 uppercase">Day {session.day}</p>
                    <p className="font-bold text-sm text-slate-700">{session.title}</p>
                  </div>
                </button>
              );
            })}
      ...
      {/* DETAIL MODAL */}
      <Modal key={activeEx?.id} open={isModalOpen} onClose={() => setIsModalOpen(false)} title={`Detail - ${activeEx?.name}`}>
        <ExerciseModalContent selectedDay={selectedDay} activeEx={activeEx} onComplete={handleExerciseCompleted} />
      </Modal>

      {/* SUCCESS POPUP (Enhanced) */}
      <Modal open={isExSuccessOpen} onClose={handleCloseExSuccess} title="">
        <ExerciseSuccessModal onClose={handleCloseExSuccess} exerciseName={activeEx?.name || "Exercise"} />
      </Modal>

      {/* SESSION COMPLETE (Tightened) */}
      <Modal open={isCongratsOpen} onClose={() => setIsCongratsOpen(false)} title="">
        <CompletionModal onClose={() => setIsCongratsOpen(false)} sessionTitle={displayedSession?.title || "Workout"} />
      </Modal>
    </div>
  );
}